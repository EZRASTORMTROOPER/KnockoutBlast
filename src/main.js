...
import { RabbitController } from "./rabbit.js";
    ...
    scene.add(player);
    const cycle = new DayNightCycle(scene);
    const rabbit = new RabbitController(scene, player);
    ...
    function update(dt){
      ...
      cycle.update(dt);
      if (cycle.isNight && !wasNight) rabbit.startNight();
      if (!cycle.isNight && wasNight) rabbit.startDay();
      rabbit.update(dt, onGround, kick, () => { playerHealth *= 0.5; });
      kick = false;
    }
    // --- Animate ---
