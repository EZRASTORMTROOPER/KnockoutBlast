...
import { keys, yaw, pitch, initControls } from './controls.js';
...
export function start() {
    ...
    function update(dt){
      ...
      const lookTarget = player.position.clone().add(new THREE.Vector3(0,1.3,0)).add(forward.clone().multiplyScalar(5));
    ...
    camera.position.set(2.5, 2.2, 12);
      const forwardInit = new THREE.Vector3(0,0,-1);
      const lookTargetInit = player.position.clone().add(new THREE.Vector3(0,1.3,0)).add(forwardInit.multiplyScalar(5));
      camera.lookAt(lookTargetInit);
