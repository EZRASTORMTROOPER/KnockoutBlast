...
import { RabbitAI } from "./rabbit.js";

// --- Renderer & Scene ---
const app = document.getElementById('app');
const healthUI = document.querySelector('.ui');
...
scene.add(sun);
const cycle = new DayNightCycle(scene, sun, hemi);
...
scene.add(player);
const rabbitAI = new RabbitAI(scene, player, cycle);
...
addEventListener('mousedown', (e) => {
  if (!pointerLocked) {
    // first click locks the pointer for camera control
    renderer.domElement.requestPointerLock();
    e.preventDefault();
  } else if (e.button === 0) {
    // left click while locked shoots or kicks
    if (rabbitAI.kick(onGround)) return;
    shoot();
  }
});
...
function update(dt){
  ...
  healthUI.textContent = "Health: " + Math.round(rabbitAI.playerHealth);
}
