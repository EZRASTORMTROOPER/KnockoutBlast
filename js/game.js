...
const GRAV = 22;
  ...
  dayNight.update(dt);
  if (dayNight.isNight) {
    if (!nightSoundPlaying && nightSound.buffer) {
      nightSound.play();
      nightSoundPlaying = true;
    }
  } else if (nightSoundPlaying) {
    nightSound.stop();
